...
class TestLogger(tune.logger.Logger):
    def _init(self):
        progress_file = os.path.join('', "hypersearchMTM"+str(2)+".csv")
...
ool['bidirectional'], params['weight_decay'], params['init_lr'], params['dropout'], params['hidden_dim'], params['filter_num'])
    ...
    with open('test1.csv','a') as outfile: #same with "w" or "a" as opening mode
    ...
    loss=-1
    with open('test1.csv', 'r', encoding='utf-8') as fp:
        for line in fp:
            print("priti", line)
...
if __name__ == '__main__':
    ...
    algo = HyperOptSearch(metric="score",
    mode="max")
...
,     "gumbel_temperature": tune.uniform(-1, 1)
 ...
 ,    "useless":tune.uniform(0,0.2)
            ...
            ,        metric="score",
        mode="max" ,resources_per_trial={'gpu': 1, 'cpu': 48}
